...
#include "typeinfo"//honglu
#include "time.h"//honglu
#include "sys/time.h"
...
void RVO2DAgent::computeNeighbors()
{
	...
	if (_RVO2DParams.rvo_max_neighbors > 0) {
		...
		dynamic_cast<RVO2DAIModule *>(rvoModule)->kdTree_->](this, _RVO2DParams.rvo_neighbor_distance * _RVO2DParams.rvo_neighbor_distance);
...
void RVO2DAgent::computeNeighbors(unsigned int frameNumber)
{
    ...
    std::cout << "Frame: "<<frameNumber<<", Current agent id: "<<this->id()<<", Current agent ray range: "<<_RVO2DParams.rvo_neighbor_distance<<", obstacleNeighbors size: " << obstacleNeighbors_.size() << std::endl;
    std::cout << "The current agent's obstacleNeighbors are: "<< std::endl;
    for (size_t i = 0; i < obstacleNeighbors_.size(); ++i)
    {
        ...
        std::cout << "obstacle_neighbor distance: "<< obstacleNeighbors_[i].first<<std::endl;
        std::cout << "obstacle_neighbor point1: "<<obstacle1->point_<<std::endl;
        std::cout << "obstacle_neighbor point2: "<<obstacle2->point_<<std::endl;
        std::cout << "obstacle_neighbor point3: "<<obstacle3->point_<<std::endl;
        std::cout << "obstacle_neighbor point4: "<<obstacle4->point_<<std::endl;
    }
    std::cout << "  "<< std::endl;
	...
	if (_RVO2DParams.rvo_max_neighbors > 0)
	{
		 ...
		 * Old ORCA method
		 */
        rangeSq = sqr(_RVO2DParams.rvo_neighbor_distance); //honglu:_RVO2DParams.rvo_neighbor_distance=10
        ...
        std::cout << "RVO finish calling computeAgentNeighbors. " << std::endl;
		/*
    ...
    //--honglu
    std::cout <<"Frame: "<<frameNumber<<", current agent "<<this->id()<<" neighbor agent info:"<< std::endl;
    //std::cout << "Frame: "<<frameNumber<<"Current agent id: "<<this->id()<<", Current agent ray range: "<<_RVO2DParams.rvo_neighbor_distance<<", agentNeighbors size: " << agentNeighbors_.size() << ", Current agent radius: "<<radius()<<", Current agent position: "<< position()<<", Current agent velocity: "<< velocity()<< std::endl;
    std::cout << "Frame: "<<frameNumber<<", Current agent id: "<<this->id()<<", Current agent ray range: "<<_RVO2DParams.rvo_neighbor_distance<<", agentNeighbors size: " << agentNeighbors_.size() << std::endl;
    std::cout << "The current agent's agentNeighbors are: "<< std::endl;
    for (size_t i = 0; i < agentNeighbors_.size(); ++i)
    {
        std::cout << "agentNeighbor "<<i<<" out of "<<agentNeighbors_.size()<<" info:"<<std::endl;
        //std::cout << "typeid(agentNeighbors_[i]).name(): "<< typeid(agentNeighbors_[i]).name()<< std::endl;
        const SteerLib::AgentInterface * other = agentNeighbors_[i].second;
        //std::cout<< "agentNeighbor id: "<<other->id()<<", distance from current agent to this neighbor agent: "<< agentNeighbors_[i].first<<", agentNeighbor radius: "<<radius()<<", agentNeighbor position: "<< position()<<", agentNeighbor velocity: "<< velocity()<<std::endl;
        std::cout<< "agentNeighbor id: "<<other->id()<<", distance from current agent to this neighbor agent: "<< agentNeighbors_[i].first<<std::endl;
        //std::cout << "RVO agent neighbor id: "<<other->id()<< std::endl;
    ...
    //--honglu
    /*
    //--honglu: rangeMaps, agentNeighbor
    for (size_t i = 0; i < agentNeighbors_.size(); ++i)
    {
  
        std::cout << "RVO agent id: "<<this->id()<<", velocity: "<<this->velocity()<< std::endl;
        Util::Vector velocity_direction = normalize(this->velocity());
        std::cout << "RVO agent id: "<<this->id()<<", velocity_direction: "<<velocity_direction<< std::endl;
...
//const SteerLib::AgentInterface * other = agentNeighbors_[i].second;
    ...
    */
    ...
    std::cout<<"nanoSecInterval,"<<tn.tv_nsec-tn1.tv_nsec<<endl;
}
...
void RVO2DAgent::draw()
{
	...
	{
		DrawLib::drawFlag( this->_currentLocalTarget, gBlack, 4.0 );
	}
