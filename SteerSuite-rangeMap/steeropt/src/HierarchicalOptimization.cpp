...
void HierarchicalOptimization::optimize()
{
	...
	/***TIME****/
	//float totalNodes = this->_steerSuite->get_nodes_count();
	//Recorder::record("total_nodes", totalNodes);
	/***TIME****/

	//SteerOpt::Recorder::instance()->printall("../results/Recorder.txt");
}
...
void HierarchicalOptimization::logOptimization(std::string fileName)
{
	...
	_logger->addDataField("time", DataType::Float);
