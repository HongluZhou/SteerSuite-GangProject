...
void Graph::saveSVGToFile(std::string filename, std::vector<std::vector<Util::Point>> queryRegions, std::vector<std::vector<Util::Point>> refRegions)
{
	double scale = 1000000.0;
	SVGBuilder svg;
	svg.style.penWidth = 0.175;
	...
	{// Works for axis-aligned bounding points
		std::vector<Util::Point> path_;
		Util::Point minPoint = refRegions[i][0];
		Util::Point maxPoint = refRegions[i][1];
		path_.push_back(minPoint);
		path_.push_back(Util::Point(minPoint.x, 0.0, maxPoint.z));
		path_.push_back(maxPoint);
		path_.push_back(Util::Point(maxPoint.x, 0.0, minPoint.z));
	...
	{// Works for axis-aligned bounding points
		std::vector<Util::Point> path_;
		Util::Point minPoint = queryRegions[i][0];
		Util::Point maxPoint = queryRegions[i][1];
		path_.push_back(minPoint);
		path_.push_back(Util::Point(minPoint.x, 0.0, maxPoint.z));
		path_.push_back(maxPoint);
		path_.push_back(Util::Point(maxPoint.x, 0.0, minPoint.z));
