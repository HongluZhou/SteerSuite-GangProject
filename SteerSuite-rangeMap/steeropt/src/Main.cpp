...
int diversityOptimizeDegreeFromFile(std::string configFileName, SteerOpt::OptimizationConfiguration optConfig)
{
	...
	// MK: This logger stores metrics calculated separately for here, not from within the optimization
	...
	if (false)
	{ // save default value
		// TODO rewrite this to not have duplicates of code.
		world->clearWorld();
		...
		double metric = steerFunc.calcMultiObjective(&best_x.front(), best_x.size()); //>>>>>>>>>>>OVERHEAD<<<<<<<<<<<<<<
		std::vector<double> metrics = steerFunc._calcMultiObjective(&best_x.front(), best_x.size()); //>>>>>>>>>>>OVERHEAD<<<<<<<<<<<<<<
	...
	else if (optConfig._objectiveFunction == "Hierarchical")
	{
		...
		for (size_t m = 0; m < steerFunc.getNumMembers(); m++)
		{
			...
			double wall_length = metrics[5];
			std::cout << "Wall length: " << wall_length << std::endl;
			std::vector<float> values = steerSuite.get_meanTreeProp(); //>>>>>>>>>>>OVERHEAD<<<<<<<<<<<<<<
			std::cout << "**** Mean Depth: " << metrics[1] << " mean Entropy: " << metrics[2] << std::endl;
			double diversity = steerFunc.getMemberDiversity(m);
			std::cout << "**** Diversity" << diversity << std::endl;
			...
			Util::Recorder::instance()->printall("../results/Recorder.txt");
