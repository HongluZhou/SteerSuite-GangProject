...
namespace Graphing {
	...
	TiXmlElement * OptimizationParameters::_composeParameter(OptimizationParameter & param)
	{
		...
		TiXmlElement * query_region_ids = new TiXmlElement("query_region_ids");
		TiXmlElement * ref_region_ids = new TiXmlElement("ref_region_ids");
		...
		for (size_t j = 0; j < param.getQueryRegionIDs().size(); j++)
		{
			TiXmlElement * q_region_id = new TiXmlElement("region");
			ss << param.getQueryRegionIDs()[j];
			TiXmlText * node_ = new TiXmlText(ss.str());
			q_region_id->LinkEndChild(node_);
			ss.str(std::string());
			query_region_ids->LinkEndChild(q_region_id);
		}

		for (size_t j = 0; j < param.getRefRegionIDs().size(); j++)
		{
			TiXmlElement * r_region_id = new TiXmlElement("region");
			ss << param.getRefRegionIDs()[j];
			TiXmlText * node_ = new TiXmlText(ss.str());
			r_region_id->LinkEndChild(node_);
			ss.str(std::string());
			ref_region_ids->LinkEndChild(r_region_id);
		}

		parameter->LinkEndChild(node_ids);
		parameter->LinkEndChild(query_region_ids);
		parameter->LinkEndChild(ref_region_ids);
	...
	void OptimizationParameters::_parseEnvParam(const ticpp::Element * root, OptimizationParameter & param)
	{
		...
		for (child = child.begin(root); child != child.end(); child++) {
			...
			else if (childTagName == "query_region_ids") {
				std::vector<size_t> query_region_ids;
				_parseRegionIDs(&(*child), query_region_ids);
				param.setQueryRegionIDs(query_region_ids);
			}
			else if (childTagName == "ref_region_ids") {
				std::vector<size_t> ref_region_ids;
				_parseRegionIDs(&(*child), ref_region_ids);
				param.setRefRegionIDs(ref_region_ids);
			}
			else {
				throw Util::GenericException("Unexpected tag <" + childTagName + "> found on line " + Util::toString(child->Row()) + "\n");
			}
	...
	void OptimizationParameters::_parseRegionIDs(const ticpp::Element * root, std::vector<size_t> & region_ids)
	{
		ticpp::Iterator<ticpp::Element> child;
		for (child = child.begin(root); child != child.end(); child++)
		{
