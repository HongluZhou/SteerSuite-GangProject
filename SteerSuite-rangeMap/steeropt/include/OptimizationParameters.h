...
namespace Graphing {
	...
	struct STEEROPTPLUG_API OptimizationParameter
	{
		...
		OptimizationParameter(double x0, double lb, double ub, std::vector<size_t> node_id, ParameterType p_type, std::vector<size_t> query_region_ids, std::vector<size_t> ref_region_ids)
		{
			_x0 = x0;
			_lb = lb;
			_ub = ub;
			...
			_rotationnM_Y = Eigen::Matrix4d::Identity();
			_name = "";
			_query_region_ids = query_region_ids;
			_ref_region_ids = ref_region_ids;
		}
		...
		// Region ids pointing to regions which are taking part in optimization
		std::vector<size_t> _query_region_ids;
		std::vector<size_t> _ref_region_ids;
		...
		virtual void setQueryRegionIDs(std::vector<size_t> in)
		{
			this->_query_region_ids.clear();
			this->_query_region_ids.insert(this->_query_region_ids.end(), in.begin(), in.end());
		}

		virtual std::vector<size_t> getQueryRegionIDs()
		{
			return this->_query_region_ids;
		}

		virtual void setRefRegionIDs(std::vector<size_t> in)
		{
			this->_ref_region_ids.clear();
			this->_ref_region_ids.insert(this->_ref_region_ids.end(), in.begin(), in.end());
		}

		virtual std::vector<size_t> getRefRegionIDs()
		{
			return this->_ref_region_ids;
		}
...
class STEEROPTPLUG_API OptimizationParameters {
	...
	private:
		...
		virtual void _parseRegionIDs(const ticpp::Element * root, std::vector<size_t> & node_ids);		
		/// Parses the steering algorithm configuration element.
