...
public:
	...
	*/
	...
	virtual void generate_visibilityGraph_cpu() { _visibilityGraph->generate_graph_gp(); }
#ifdef CUDA_ENABLED
	...
	virtual void generate_forest_gpu() { _visibilityGraph->generate_forest_cuda(); }
	...
	virtual float get_nodes_count() { return _visibilityGraph->get_nodeList().size(); }
	// get total number of ostacles
	virtual float get_obs_count() { return _visibilityGraph->get_obstacles().size(); }
