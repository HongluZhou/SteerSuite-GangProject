...
#else
	this->setConfigFileName("../config_unix.xml");
#endif
}
...
void SteerSuite::init_visibilityGraph(Util::Point pMin, Util::Point pMax, unsigned int gridNumX, unsigned int gridNumZ, float height)
{
	...
	std::cout << "===========FOR CRYING OUT LOAD >> " << gridNumX << "_" << gridNumZ << std::endl;
}
...
void SteerSuite::setup_visibilityGraph()
{
	...
	for (auto itr = obs.begin(); itr != obs.end(); itr++)
	{
		...
		//obstacleList.push_back(Obstacle(pMin, pMax, 0, ObstacleType::BLOCK)); //this is for axis aligned box
		if ((*itr)->isStatic())
			type = SpaceSyntax::ObstacleType::STATIC;
		else
			type = SpaceSyntax::ObstacleType::BLOCK;
		obstacleList.push_back(Obstacle(obstaclePoints[0], obstaclePoints[1], obstaclePoints[2], obstaclePoints[3], (*itr)->getBounds().ymin, (*itr)->getBounds().ymax, type));
	}
	_visibilityGraph->clear_obstacles();
	_visibilityGraph->add_obstacles(obstacleList);
	//_visibilityGraph->generate_graph_gp(); //legacy setup where generate was done from within setup, new version requires generate_graph to run separately afterwards
}
