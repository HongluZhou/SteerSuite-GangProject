...
void SimWorld::addOrientedObstacle(Util::Point centerPosition, float lengthX, float lengthZ, float ymin, float ymax, float thetaY, bool isStatic)
{
	// std::cout << "SimWorld adding OrientedObstacle: " << centerPosition << " theta: " << thetaY << std::endl;
	SteerLib::OrientedBoxObstacle * box = new SteerLib::OrientedBoxObstacle(centerPosition, lengthX, lengthZ,	ymin, ymax, thetaY, 1001.0f, isStatic);
	/*
