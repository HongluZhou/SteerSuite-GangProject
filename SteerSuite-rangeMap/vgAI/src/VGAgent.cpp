...
void VGAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
	...
	for (auto itr = obs.begin(); itr != obs.end(); itr++)
	{
		...
		*/
		...
		obstacleList.push_back(Obstacle(obstaclePoints[0], obstaclePoints[1], obstaclePoints[2], obstaclePoints[3], (*itr)->getBounds().ymin, (*itr)->getBounds().ymax, SpaceSyntax::ObstacleType::BLOCK));
	}
	...
	vg = new VisibilityGraph(-10, -10, 15, 10, 0.1, 25, 20);
	vg->add_obstacles(obstacleList);
	vg->add_region_gp(-10, -10, 14.99, 9.99, RegionType::REF);
	vg->add_region_gp(-10, -10, 14.99, 9.99, RegionType::QUERY);
	...
	//vg->add_region_gp(5, -2, 9, 2, RegionType::QUERY);
	//vg->add_region_gp(6, 6, 9, 9, RegionType::REF);
	//vg->add_region_gp(5, 7, 6, 9, RegionType::QUERY);
	//vg->add_region_gp(7, -10, 9, -8, RegionType::REF);

	float mDegree, mDepth, mEntropy;
	//============GPU
	
	vg->generate_graph_cuda();
	vg->generate_forest_cuda();
	vg->get_output_cuda(mDegree, mDepth, mEntropy);
	vg->fetch_graph_cuda();
	
	//=========CPU

	//vg->generate_graph_gp();
	//mDegree = vg->get_meanDegree();
	//mDepth = vg->get_meanTreeProp().first;
	//mEntropy = vg->get_meanTreeProp().second;
	

	//std::cout << "CHECK CONN====" << vg->check_connection() << std::endl;
	std::cout << "Degree" << mDegree << std::endl;
	std::cout << "Depth" << mDepth << std::endl;
	std::cout << "Entropy" << mEntropy << std::endl;
	*/
	/*****ANALYSIS PART*****/
	
	clock_t begin, b1, b2;
	unsigned runNum = 5, gridNum = 12;
	std::ofstream afile, dfile, efile, tfile, tgfile, ttfile;
	//std::string aname = "gpu_degree.txt", dname = "gpu_depth.txt", ename = "gpu_entropy.txt", tname = "gpu_time.txt", tgname = "gpu_time_graph.txt", ttname = "gpu_time_tree.txt";
	std::string aname = "cpu4_degree.txt", dname = "cpu4_depth.txt", ename = "cpu4_entropy.txt", tname = "cpu4_time.txt", tgname = "cpu4_time_graph.txt", ttname = "cpu4_time_tree.txt";
	// clear files
	...
	efile.close();
	tfile.open(tname, std::ofstream::out | std::ofstream::trunc);
	tfile.close();
	tgfile.open(tgname, std::ofstream::out | std::ofstream::trunc);
	tgfile.close();
	ttfile.open(ttname, std::ofstream::out | std::ofstream::trunc);
	ttfile.close();
	...
	tfile.open(tname, std::ofstream::out | std::ofstream::app);
	tgfile.open(tgname, std::ofstream::out | std::ofstream::app);
	ttfile.open(ttname, std::ofstream::out | std::ofstream::app);

	vg = new VisibilityGraph(-10, -10, 10, 10, 0.1, 10, 10); //dummy
	vg->add_obstacles(obstacleList);
	float gran = 0;
	for (unsigned j = 0; j < gridNum+1; j++)
	{
		gran += 0.25;
		...
		vg->setup_gridPlan(-10, -10, 10, 10, 0.1, 20 * gran, 20 * gran);
		vg->add_region_gp(-10, -10, 9.9999, 9.9999, RegionType::REF);
		vg->add_region_gp(-10, -10, 9.9999, 9.9999, RegionType::QUERY);
		
		vg->add_obstacles(obstacleList);
		_nodeList = vg->get_nodeList();
		afile << _nodeList.size() << " ";
		dfile << _nodeList.size() << " ";
		efile << _nodeList.size() << " ";
		tfile << _nodeList.size() << " ";
		tgfile << _nodeList.size() << " ";
		ttfile << _nodeList.size() << " ";
		//afile << gran << " ";
		//dfile << gran << " ";
		//efile << gran << " ";
		for (unsigned i = 0; i < runNum; i++)
		{
			...
			begin = clock();
			vg->generate_graph_cuda();
			tg = ((double)1000 * (clock() - begin)) / CLOCKS_PER_SEC;
			b1 = clock();
			vg->generate_forest_cuda();
			vg->get_output_cuda(mDegree, mDepth, mEntropy);
			tt = ((double)1000 * (clock() - b1)) / CLOCKS_PER_SEC;
			t = ((double)1000 * (clock() - begin)) / CLOCKS_PER_SEC;
			vg->fetch_graph_cuda();
			*/
			//=========CPU
			...
			mDepth = treeProp.first;
			mEntropy = treeProp.second;
			tt = ((double)1000 * (clock() - b1)) / CLOCKS_PER_SEC;
			t = ((double)1000 * (clock() - begin)) / CLOCKS_PER_SEC;
