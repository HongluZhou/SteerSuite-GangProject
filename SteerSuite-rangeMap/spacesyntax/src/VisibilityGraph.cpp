...
void VisibilityGraph::init_graph()
{
	...
	_gpuvg.clear();
#endif
	// sort _nodeList so ref points are placed first, followed by ref/query nodes, and then query only nodes
	vector<Node> rqNodes, qNodes, rNodes;
	int refCount = get_ref_only_count();

	for (auto itr = _queryRegion.begin(); itr != _queryRegion.end(); itr++)
	{
		if (_nodeList[itr->second].isRef)
			continue;
		// node is just query, not reference
		qNodes.push_back(_nodeList[itr->second]);
		itr->second = _refRegion.size() + qNodes.size() - 1;
	}

	for (auto itr = _refRegion.begin(); itr != _refRegion.end(); itr++)
	{
		if (_nodeList[itr->second].isQuery)
		{
			// node is  both reference and query
			rqNodes.push_back(_nodeList[itr->second]);
			auto qitr = _queryRegion.find(itr->first);
			if (qitr == _queryRegion.end())
				throw Util::GenericException("VisGraph: query Region is not filled correctly.");
			qitr->second = rqNodes.size() + refCount - 1;
			itr->second = rqNodes.size() + refCount - 1;
		}
		else
		{
			// node is just reference, not query
			rNodes.push_back(_nodeList[itr->second]);
			itr->second = rNodes.size() - 1;
		}
	}

	auto enditr = std::copy(rNodes.begin(), rNodes.end(), _nodeList.begin());
	enditr = std::copy(rqNodes.begin(), rqNodes.end(), enditr);
	enditr = std::copy(qNodes.begin(), qNodes.end(), enditr);
	if (enditr == _nodeList.end())
		cout << ">>>>>>CORRECT SORTING OF NODELIST<<<<<<<" << endl;
	cout << ">>>>>>>>>IN_INIT_GRAPH<<<<<<<<<<<<<<<" << endl;

	Util::Recorder::instance()->record("9querySize", _queryRegion.size());
	Util::Recorder::instance()->record("9refSize", _refRegion.size());
	Util::Recorder::instance()->record("9totalNodes", _nodeList.size());
	Util::Recorder::instance()->record("9totalObs", get_obstacles().size());
}
...
{
	_isInit = false;
	// moved this to init_graph to avoid multiple calls to clear
//#ifdef CUDA_ENABLED
//	_gpuvg.clear();
//#endif
...
void VisibilityGraph::generate_graph_cuda()
{
	...
	{
		if (obstacles[i].get_Type() == ObstacleType::STATIC)
			continue;
		Util::Point tmp;
	...
	if (!_gpuvg.is_init())
	{
		...
		_gpuvg.init(gpuPoints, obstacles.size(), _refRegion.size(), _queryRegion.size(), get_ref_only_count());
		cout << ">>>>>>>>>>>>>>>>>>*****GPU_INIT******<<<<<<<<<<<<<<<<<" << endl;

		//Fill static obstacles default adjMatrix in gpu
		vector<Obstacle_Struct> obsListDef;
		for (int i = 0; i < obstacles.size(); i++)
		{
			if (obstacles[i].get_Type() == ObstacleType::BLOCK)
				continue;
			Util::Point tmp;
			Point_Struct obsP1, obsP2;
			tmp = obstacles[i].get_pMin();
			...
			tmp = obstacles[i].get_pMax();
			obsP2.x = tmp.x; obsP2.y = tmp.y; obsP2.z = tmp.z;
			obsListDef.push_back(Obstacle_Struct(obsP1, obsP2, obstacles[i].get_Type(), obstacles[i].get_tilt()));
			_gpuvg.generate_graph(obsListDef, ObstacleType::STATIC);
		}
		Util::Recorder::instance()->record("9staticObs", obsListDef.size());
	}
...
#pragma omp parallel for
	...
	for (int i = 0; i < matSize; i++)
		for (int j = i; j < matSize; j++)
		{
			_adjMatrix(i, j) = adjMatrix[i*matSize + j] == false ? 1 : 0; // false is connected in gpu
...
void VisibilityGraph::get_output_cuda(float& mDegree, float& mDepth, float& mEntropy)
{
	...
	//std::cout << "GPU>>>>>>>>>>>>" << mDegree << "_" << mDepth << "_" << mEntropy << std::endl;
	free(gpu_output);
}
