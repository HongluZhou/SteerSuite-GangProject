...
inline void evaluateParabolaAtTime(const Footstep & step, float currentTime, Util::Point & position, Vector & velocity)
{
	...
	float currentLocalStepTime = 0.0;
	switch (step.state) {
...
inline void drawFoot(float x, float z, float orientation, bool whichFoot, const float alpha=1.0f) {
	...
	{
		if (whichFoot==LEFT_FOOT)
			DrawLib::setColour (1.0f, 0.8f, 0.2f, alpha); // yellow
		else
			DrawLib::setColour(0.4f, 0.6f, 1.0f, alpha); // cyan
	}

	DrawLib::glPushMatrix(); {
		DrawLib::glTranslate(x, 0.0f, z);
		DrawLib::glRotate(-orientation * M_180_OVER_PI, 0.0f, 1.0f, 0.0f);
		//glScalef(0.05f, 0.05f, 0.05f);
		//DrawLib::drawCube();
		DrawLib::glScale(0.2f, 0.0005f, 0.07f);
		//DrawLib::drawCube();
		...
		// draw the arrow
		const float h = 0;
		DrawLib::glScale(1.0 / 0.2f * 0.1f, 1.0 / 0.0005f, 1.0 / 0.07f * 0.1f); // undo the previous scale and scale the arrow
		DrawLib::setColour(0.3f, 0.3f, 0.3f, alpha);
...
inline void drawFootstepTrajectory(const Footstep & step, const Util::Color & color, const float alpha=1.0f) {
	...
	if (alpha == 1.0)
		...
		DrawLib::setColour( color.r, color.g, color.b, alpha);

	int npoints = (int)((step.endTime - step.startTime) / increment + 0.5) + 2 ;  // 0.5 for rounding and +2 for safety
	Point *lp = new Point[npoints];
	if (lp == NULL) {
		cerr << "ERROR: drawFootstepTrajectory Cannot allocate memory\n" ;
		exit(-1);
	}
	...
	DrawLib::DrawLineStrip(lp, count);
...
void FootstepAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
	...
	//gSpatialDatabase->getLocationFromIndex(_midTermPath[_midTermPathSize-1],midtermPathStartPoint);
///////////////////////////////////////////////////////////////////////////////
...
void FootstepAgent::_planFootstepsToShortTermGoal(bool useHardCodedPlan)
{
	if (!useHardCodedPlan) {
		...
		if (searchAlgorithm.getPath().size() == 0) {
			...
			if (gEngine->isAgentSelected(this)) ////////////////////////////////////////////////////////// DEBUGGING
			{
				std::cout << "_planFootstepsToShortTermGoal:\tsearchAlgorithm.getPath().size(): " << searchAlgorithm.getPath().size() << "\n";
			} 
#endif
			...
			if (gEngine->isAgentSelected(this)) ////////////////////////////////////////////////////////// DEBUGGING
			{
				std::cout << "_planFootstepsToShortTermGoal:\t_footstepPlan.size(): " << _footstepPlan.size() << "\n";
			} 
		...
		for (unsigned int step = 1; step < 14; step++) {
			...
			if (step == 1) {
				if (!_createFootstepAction(0.6f, parabolaPhiStraight + 0.05f*radiansInterval, false, PREFERRED_SPEED, prevStep, nextStep, FOOTSTEP_STATE_NORMAL) == true) {
					std::cerr << "footstep 1 not created.\n";
					break;
				}
			}
			else if (step == 2) {
				if (!_createFootstepAction(0.7f, parabolaPhiStraight + 0.05f*radiansInterval, false, PREFERRED_SPEED, prevStep, nextStep, FOOTSTEP_STATE_NORMAL) == true) {
					std::cerr << "footstep 2 not created.\n";
					break;
				}
			}
			else if (step == 3) {
				if (!_createFootstepAction(0.8f, parabolaPhiStraight + 0.05f*radiansInterval, false, PREFERRED_SPEED, prevStep, nextStep, FOOTSTEP_STATE_NORMAL) == true) {
					std::cerr << "footstep 3 not created.\n";
					break;
				}
			}
			else if (step == 10) {
				if (!_createFootstepAction(0.9f, parabolaPhiStraight + 0.05f*radiansInterval, false, PREFERRED_SPEED, prevStep, nextStep, FOOTSTEP_STATE_NORMAL) == true) {
					std::cerr << "footstep 3 not created.\n";
					break;
				}
			}
...
void FootstepAgent::_updateCharacterFollowingPath(float currentTime)
{
	...
	Util::Point endposition = Util::Point(_currentStep.outputCOMState.x, 0.0f, _currentStep.outputCOMState.z);
	...
	// replan before last step is taken (except when reaching goal). This is forward thinking in the case the last step is a poor situation
		if ( ((_currentLocationInPath == _footstepPlan.size()-1) && _footstepPlan.size() != 1) ||
#ifdef ROBUST_FOOTSTEPS
				...
				(_currentLocationInPath > ped_num_steps_before_forced_plan))
		{
			...
			if (gEngine->isAgentSelected(this)) ////////////////////////////////////////////////////////// DEBUGGING
			{
				std::cout << "_updateCharacterFollowingPath:\tInside condition!\n";
				std::cout << "_currentLocationInPath = " << _currentLocationInPath<<"\n";
				std::cout << "_cachedFootstepOptions[_footstepPlan[_currentLocationInPath]].endTime = " << _cachedFootstepOptions[_footstepPlan[_currentLocationInPath]].endTime << "\n";
				std::cout << "currentTime = " << currentTime << "\n";
				std::cout << "((_currentLocationInPath == _footstepPlan.size()-1)):\t" << ((_currentLocationInPath == _footstepPlan.size()-1)) << "\n";
				std::cout << "((_currentLocationInPath == _footstepPlan.size()-2) && (!_cachedFootstepOptions[_footstepPlan[_footstepPlan.size()-1]].isAGoalState)):\t" << ((_currentLocationInPath == _footstepPlan.size()-2) && (!_cachedFootstepOptions[_footstepPlan[_footstepPlan.size()-1]].isAGoalState)) << "\n";
				std::cout << "(_currentLocationInPath > ped_num_steps_before_forced_plan)):\t" << (_currentLocationInPath > ped_num_steps_before_forced_plan) << "\n";
			}
#endif
			if (!_enabled) return;
			_needToRunFootstepPlanner = true;
		}
		else if (_currentLocationInPath + 1 < _footstepPlan.size()) 
		{
			_stepHistory.push_back(_currentStep);
			...
			{
				// If we lose sight of a target (concavity@)
				// Redo midterm planning
				// Fix local target to the closest visible one
				// Call for footstep replan
#ifdef _DEBUG
					if (gEngine->isAgentSelected(this)) ////////////////////////////////////////////////////////// DEBUGGING
					{
						std::cout << "_updateCharacterFollowingPath:\tLost sight of local target!\n";
					}
#endif
					...
					for (int i = _currentWaypointIndex; _midTermPathSize - i - 1 > 0; i++) 
					{
						...
						gSpatialDatabase->getLocationFromIndex(_midTermPath[_midTermPathSize - i - 1],tempPathStartPoint);
					...
					Util::Point midtermPathStartPoint;
					gSpatialDatabase->getLocationFromIndex(_midTermPath[_midTermPathSize - _currentWaypointIndex - 1],midtermPathStartPoint);
					_localTargetLocation = midtermPathStartPoint;
#ifdef _DEBUG
					if (gEngine->isAgentSelected(this)) ////////////////////////////////////////////////////////// DEBUGGING
					{
						std::cout << "_updateCharacterFollowingPath:\tNew local Target: " << _localTargetLocation << "\n";
					}
#endif
				}
#ifdef _DEBUG
...
#endif
		} else if (_footstepPlan.size() == 1 && (  (endposition - _currentSimulatedPosition).lengthSquared() < 0.0000001 ))
		{
			//_runLongTermPlanningPhase();
			//_runMidTermPlanningPhase();
			_needToRunFootstepPlanner = true;
			...
			if (gEngine->isAgentSelected(this)) ////////////////////////////////////////////////////////// DEBUGGING
			{
				std::cout << "_updateCharacterFollowingPath:\tfootstep.state" << _currentStep.state << "\n";
				std::cout << "_updateCharacterFollowingPath:\tfootstep plan is size 1\n";
				std::cout << "_cachedFootstepOptions[_footstepPlan[_currentLocationInPath]].endTime " << _cachedFootstepOptions[_footstepPlan[_currentLocationInPath]].endTime  << "\n";
				std::cout << "currentTime " << currentTime << "\n";
			}
#endif
...
void FootstepAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	if ( ! _midTermPath.empty() && (!this->hasLineOfSightTo(goalInfo.targetLocation)) )
	{
		...
		Util::Point midtermPathStartPoint;
		if ((position() - _localTargetLocation).lengthSquared() <= (ped_reached_target_distance_threshold*ped_reached_target_distance_threshold)) 
		{
			//this->updateMidTermPath();
#ifdef _DEBUG
			if (gEngine->isAgentSelected(this)) 
			{
				std::cout << "updateAI:\tHit target\n";
			}
#endif 
	...
	if (_needToRunFootstepPlanner) {
		...
		if (gEngine->isAgentSelected(this))
		{
			std::cout << "updateAI:\tNeed to run fotsteps planner!\n";
		}
#endif
		_runPerceptivePhase();
...
void FootstepAgent::_runMidTermPlanningPhase()
{
	...
	if (_midTermPathSize > (next_waypoint_distance + 3)) {
		...
		std::cerr << "ERROR!!!  _midTermPathSize is larger than expected: should be less than or equal to " << next_waypoint_distance+3 << 
		", but it actually is " << _midTermPathSize << "\n";
		//std::cerr << "exiting ungracefully.\n";
		//exit(1);
	}*/
...
void FootstepAgent::_runShortTermPlanningPhase()
{
...
#endif
	closestPathNode = _midTermPath.size();
#ifdef _DEBUG_1
	std::cout << "In _runShortTermPlanningPhase, closestPathNode index: " << _midTermPathSize << "\n";
#endif
	if ((myIndexPosition != -1)&&(closestPathNode != 0))// myIndexPosition is uninitialized
	{
		...
		int i=_midTermPathSize-1;
		for (std::vector<int>::reverse_iterator rit = _midTermPath.rbegin(); rit != _midTermPath.rend(); ++rit)
		{
			Util::Point tempTargetLocation;
#ifdef _DEBUG_1
			std::cout << "In _runShortTermPlanningPhase, Mid term index: " << i << " = " << *rit << "\n";
#endif
			gSpatialDatabase->getLocationFromIndex( *rit, tempTargetLocation);
...
#ifdef _DEBUG_1
		std::cout << "In _runShortTermPlanningPhase, upated closestPathNode: " << closestPathNode << " = " << _midTermPath.at(closestPathNode) << "\n";
#endif
		...
		if (closestPathNode > 2)
		{
			...
			int localTarget = _midTermPath.at(localTargetIndex);
#ifdef _DEBUG_1
			std::cout << "In _runShortTermPlanningPhase, step1" << "\n";
#endif
			...
			{
#ifdef _DEBUG_1
				std::cout << "In _runShortTermPlanningPhase, localTargetIndex: " << localTargetIndex << "\n";
#endif
				localTarget = _midTermPath.at(localTargetIndex);

#ifdef _DEBUG_1
				std::cout << "In _runShortTermPlanningPhase, step2" << "\n";
#endif
				gSpatialDatabase->getLocationFromIndex( localTarget, _localTargetLocation );
				lineOfSightTest1.initWithUnitInterval(_position + _radius*rightSide, _localTargetLocation - (_position + _radius*rightSide));
				lineOfSightTest2.initWithUnitInterval(_position - _radius*rightSide, _localTargetLocation - (_position - _radius*rightSide));
			...
			if (localTargetIndex <= closestPathNode) {
				...
				localTarget = _midTermPath.at(localTargetIndex);
#ifdef _DEBUG_1
				std::cout << "In _runShortTermPlanningPhase, step3" << "\n";
#endif
				gSpatialDatabase->getLocationFromIndex( localTarget, _localTargetLocation );
				...
				localTarget = _midTermPath.at(closestPathNode-2);
#ifdef _DEBUG_1
				std::cout << "In _runShortTermPlanningPhase, step4" << "\n";
#endif
				gSpatialDatabase->getLocationFromIndex( localTarget, _localTargetLocation );
			}
		}
