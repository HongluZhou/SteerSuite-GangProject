...
#include "dataAI/FootstepEnvironment.h"
#include "SteerLib.h"
#include <math.h>
...
volatile unsigned int DataAgent::numPlanner = 0;
volatile unsigned int DataAgent::numData = 0;

//C++ standard makes me do this for a const float that isn't global
const float DataAgent::CONFIDENCE_THRESHOLD = 0.35f;
...
DataAgent::DataAgent(TreeInterface* trees, StateConfig* states, bool restrictedContext, unsigned int whichContext, bool dumpTrans, std::vector<std::string>* trans)
{
	firstRun = true;
	classifier = trees;
	featureSets = states;
	_enabled = false;
	soleContext = whichContext;
	constrainContext = restrictedContext;
	outputTranscript = dumpTrans;
	transcript = trans;
}
...
{
	#ifdef _DEBUG
	DataAgent::numPlanner = DataAgent::numPlanner;
	DataAgent::numData = DataAgent::numData;
	#endif
}

void DataAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
	//
	//
...
bool DataAgent::_createFootstepAction(float stepDuration, float parabolaOrientationPhi, bool phiIsIdeal, float desiredVelocity, const Footstep & previousStep, Footstep & nextStep, FootStateEnum nextState, bool forced) const
{
	...
	if (localDz > 0.0f && !forced) {
		//std::cerr << "####  DataAgent::_createFootstepAction() - pendulum swinging in wrong direction (away from foot)!  localDz = " << localDz << "\n";
		return false;
	}
	...
	//if (localZ < 0.0f) {
		...
		std::cerr << "####  DataAgent::_createFootstepAction() - invalid position condition for this desired step!\n";
		assert(false);
	}
	...
	*/
	...
	if ((nextState == FOOTSTEP_STATE_NORMAL) || (nextState == FOOTSTEP_STATE_STOPPING)) {
		...
		if (stepDistanceSquared < _walkParameters.minStepLength*_walkParameters.minStepLength && !forced) {
			//std::cerr << "####  DataAgent::_createFootstepAction() - cannot create this step, it is too small!  " << sqrtf(stepDistanceSquared) << " < " << _walkParameters.minStepLength << "\n";
			return false;
		}

		if (stepDistanceSquared > _walkParameters.maxStepLength*_walkParameters.maxStepLength && !forced) {
			//std::cerr << "####  DataAgent::_createFootstepAction() - cannot create this step, it is too large!  " << sqrtf(stepDistanceSquared) << " > " << _walkParameters.maxStepLength << "\n";
			return false;
		}
	}
	...
	for (float interp = 0.05f; interp <= 1.0f; interp += 0.09f) {
		...
		for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin();  neighbor != _neighbors.end();  ++neighbor) {
			if ((*neighbor)->isAgent()) {
				...
				DataAgent * agent;
				agent = dynamic_cast<DataAgent*>(*neighbor);
...
void DataAgent::_planFootstepsToShortTermGoal(bool useHardCodedPlan)
{
	if (!useHardCodedPlan) {
		...
		if(!constrainContext) {
			...
			contextResult.first = soleContext;
			contextResult.second = 0.5f;
		}
		...
		if(isActionValid) {
			...
			if(outputTranscript) {
				...
				strstrm << contextResult.first << '\t' << contextResult.second << '\t' << specializedResult.second << '\n';
				transcript->push_back(strstrm.str());
			}
		}
		else
		{
...
void DataAgent::_updateCharacterFollowingPath(float currentTime)
{
...
void DataAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
...
bool DataAgent::collidesAtTimeWith(const Point & p1, const Vector & rightSide, float otherAgentRadius, float timeStamp, float otherAgentFootX, float otherAgentFootZ)
{
...
void DataAgent::addGoal(const SteerLib::AgentGoalInfo & newGoal) { 
...
// reachedCurrentGoal()
//
bool DataAgent::_reachedCurrentGoal()
{
...
//
bool DataAgent::_reachedCurrentWaypoint()
{
...
// reachedLocalTarget()
//
bool DataAgent::_reachedLocalTarget()
{
...
// runCognitivePhase()
//
void DataAgent::_runCognitivePhase()
{
	//assert(_landmarkQueue.front() == _currentGoal);
...
//
void DataAgent::_runLongTermPlanningPhase()
{
...
//
void DataAgent::_runMidTermPlanningPhase()
{
...
//
void DataAgent::_runShortTermPlanningPhase()
{
	int closestPathNode;
	if (!_enabled) return;
...
// runPerceptivePhase()
//
void DataAgent::_runPerceptivePhase()
{
	if (!_enabled) return;
...
void DataAgent::dumpNearbyInfo(vector<agentInfo>& agents, vector<obstacleInfo>& obstacles, bool isContext) {
	agents.clear();
	obstacles.clear();
	...
	for(set<SpatialDatabaseItemPtr>::const_iterator citer = listRef.cbegin(); citer != listRef.cend(); ++citer) {
		if((*citer)->isAgent()) {
			...
			agent = static_cast<DataAgent*>(*citer);
...
void DataAgent::handleSlice(const StateConfig::slice wedge, vector<agentInfo>& agents, vector<obstacleInfo>& obstacles, stringstream& result) {
...
void DataAgent::handleDensity(const StateConfig::density area, vector<agentInfo>& agents, vector<obstacleInfo>& obstacles, stringstream& result) {
...
void DataAgent::handleFlow(const StateConfig::flow area, vector<agentInfo>& agents, stringstream& result) {
	unsigned int counter = 0;
...
void DataAgent::handleObstacles(const StateConfig::obstaclesPresent, vector<obstacleInfo>& obstacles, stringstream& result) {
...
string DataAgent::extractContextSamples(vector<agentInfo>& agents, vector<obstacleInfo>& obstacles) {
...
string DataAgent::extractSpecializedSamples(unsigned int contextNumber, vector<agentInfo>& agents, vector<obstacleInfo>& obstacles) {
...
void DataAgent::localSpaceXForm(vector<agentInfo>& agents, vector<obstacleInfo>& obstacles) {
	float subjectTheta = atan2f(_currentStep.outputCOMState.dz, _currentStep.outputCOMState.dx);
