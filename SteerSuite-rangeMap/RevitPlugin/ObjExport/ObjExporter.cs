...
namespace ObjExport
{
  class ObjExporter : IJtFaceEmitter
  {
    ...
    static double ConvertFeetToMetres(double d)
    {
        return (_feet_to_meters * d );
    }
    ...
    class VertexLookupInt : Dictionary<PointInt, int>
    {
      ...
      public int AddVertex( PointInt p )
      {
        ...
        {
            #region PointDoubleEqualityComparer
            /// <summary>
            /// Define equality for integer-based PointInt.
            /// </summary>
    ...
    public ObjExporter()
    {
      _faceCount = 0;
      _triangleCount = 0;
      _vertices = new VertexLookupDouble();
    ...
    void StoreTriangle( MeshTriangle triangle )
    {
      for( int i = 0; i < 3; ++i )
      {
        ...
        PointDouble q = new PointDouble(p);
    ...
    public void ExportTo(string path)
    {
        ...
        using (StreamWriter s = new StreamWriter(path))
        {
            ...
            foreach (PointDouble key in _vertices.Keys)
            {
                EmitVertex(s, key);
            }
      ...
      public void ExportToSteerSuite()
    {
...
/*
        ...
        */
        // using (StreamWriter s = new StreamWriter(path))
        {
            double[] points = new double[_vertices.Keys.Count * 3];
            UInt64 j = 0;
           foreach (PointDouble key in _vertices.Keys)
            {
                points[j] = key.X;
            ...
            int i = 0;
            int n = _triangles.Count;
           UInt64[] faces = new UInt64[_triangles.Count];
