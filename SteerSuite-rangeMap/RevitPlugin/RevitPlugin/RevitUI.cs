...
namespace RevitPlugin
{
    ...
    class RevitUI : IExternalEventHandler
    {
        ...
        private string NODES_FILENAME = "OptimizationNodes";
        ...
        /// <returns></returns>
        public System.Windows.Forms.Form InitializeForm(Document doc, ExternalEvent m_ExEvent, bool usMode, bool dev)
        {
            this.doc = doc;
        ...
        public void UIForm_Closing(object sender, FormClosingEventArgs e)
        {
            if (this.userstudyMode || this.userstudy_A)
            {
                ...
                {
                    newResultLine = string.Format("{0},{1}", "STUDY(A/B/C)", "TOTAL_TIME");
                    results.AppendLine(newResultLine);

                    if (this.userstudy_A)
                        newResultLine = string.Format("{0},{1}", "A", (elapsedMs / 1000));
                    else if (this.userstudy_B)
                        newResultLine = string.Format("{0},{1}", "B", (elapsedMs / 1000));
                    else
                        newResultLine = string.Format("{0},{1}", "C", (elapsedMs / 1000));
                    results.AppendLine(newResultLine);
                }
                else
                {
        ...
        public void HelperSelectSpaceRegion(XYZ rMin, XYZ rMax)
        {
            ...
            this.worldBoundMin = new XYZ(xMin, 0, yMin) * _feet_to_meters; // top left
            this.worldBoundMax = new XYZ(xMax, 0, yMax) * _feet_to_meters; // bottom right
            ...
            else if (this.radioRegionReference.Checked)
            {
                ...
                _rf.Pinned = true;
                ChangeElementColor(this.doc, _rf.Id, new Autodesk.Revit.DB.Color(192, 192, 192));
        ...
        public void comboMetricSelection_Click(object sender, EventArgs e)
        {
            ...
            if (this.steersuiteLayout == null)
            {
                ...
                steersuite.generate_visibilityGraph_cpu();
            }
            else
            {
        ...
        public void ProcessSpaceGraph()
        {
            ...
            StoreGraph(this.doc, this.pathToSteerSuite + "_walls.graph");
            Graph _walls = new Graph();
            _walls.importFromFile(this.pathToSteerSuite + "_walls.graph");

            for (UInt64 edge = 0; edge < _walls.num_edges(); edge++)
            {
                // Eigen::Vector3d cp_ = (_walls.nodes.at(_walls.edges[edge]._origin) + _walls.nodes.at(_walls.edges[edge]._end)) / 2.0;
                DoubleVector n0 = _walls.getNode(_walls.get_edge((uint)edge)._origin);
                DoubleVector n1 = _walls.getNode(_walls.get_edge((uint)edge)._end);
                ...
                // Eigen::Vector3d edge_ = (_walls.nodes.at(_walls.edges[edge]._end) - _walls.nodes.at(_walls.edges[edge]._origin)).normalized();
        ...
        public SteerSuite ProcessSpaceGraph2(Graph graph)
        {
            ...
            ss.generate_visibilityGraph_cpu();
            return ss;
        }
        ...
        public DoubleVector GetDefaultMetricsValues(Graph g)
        {
            ...
            //if no reference region is specified then consider query region also as reference region
            if (this.studyParams._refRegions.Count < 1)
            {
                for (int r = 0; r < this.studyParams._queryRegions.Count; r++)
                {
                    steerFunc._steerSuite.add_refRegion(this.studyParams._queryRegions[r][0], this.studyParams._queryRegions[r][1]);
                }
            }
            else
            {
        ...
        public void ok_Click(object sender, EventArgs e)
        {
            ...
            if(this.userstudy_B)
                steerFunc.setDiversityMembers(1); // just return a single optimal solution
            else
                steerFunc.setDiversityMembers(this.optConfig._diversity_members); // retrun diverse set of near optimal solutions (i.e. 5)
            ...
            if (this.userstudyMode)
            {
                ...
                pathToNodes = this.pathToParticipant;
            ...
            //storing which nodes are optimized in the current iteration
            StringBuilder opt_nodes_count = new StringBuilder();
            pathToNodes = pathToNodes + Path.DirectorySeparatorChar + NODES_FILENAME + OPT_OUTPUT_FILETYPE;
            string nodes_data_header = string.Format("{0},{1},{2}", "Participant_Id", "Optimization_Iteration", "Node_Id");
            opt_nodes_count.AppendLine(nodes_data_header);
            ...
            for (int i = 0; i < this.params_.size(); i++)
            {
                ...
                {                    
                    foreach (uint n in nodes)
                    {
                        String node_data = "";
                        if (!this.userstudyMode)
                            node_data = string.Format("{0},{1},{2}", "DEV", this.participantTurns.ToString(), n.ToString());
                        else
                            node_data = string.Format("{0},{1},{2}", this.partcipantId.ToString(), this.participantTurns.ToString(), n.ToString());
                        opt_nodes_count.AppendLine(node_data);
                    }
                }
            }
            ...
            try
            {
                File.AppendAllText(pathToNodes, opt_nodes_count.ToString());
            }
            catch (Exception ex)
            {
                TaskDialog.Show("uDOME", "Exception in Writing Optimization Nodes to File. " + ex.ToString());
            }
        ...
        public void CreateAnalysisDisplayStyle(Document doc)
        {
            ...
            colors.Add(new AnalysisDisplayColorEntry(new Autodesk.Revit.DB.Color(0, 255, 255)));
